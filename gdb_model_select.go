...
// all (原All) does "SELECT FROM ..." statement for the model.
...
// one (原One) retrieves one record from table and returns the result as map type.
// It returns nil if there's no record retrieved with the given conditions from table.
//
// The optional parameter <where> is the same as the parameter of Model.Where function,
// see Model.Where.
...
// value (原Value) retrieves a specified record value from table and returns the result as interface type.
...
// array (原Array) queries and returns data values as slice from database.
...
// doStruct (原Struct) retrieves one record from table and converts it into given struct.
...
// doStructs (原Structs) retrieves records from table and converts them into given struct slice.
...
// scan (原Scan)automatically calls Struct or Structs function according to the type of parameter <pointer>.
...
// Take (原FindOne) retrieves and returns a single Record by Model.WherePri and Model.One.
...
// Find (原FindAll) retrieves and returns Result by by Model.WherePri and Model.All.
...
// Count (原FindCount) retrieves and returns the record number by Model.WherePri and Model.Count.
...
// Scan (原FindScan) automatically calls Struct or Structs function according to the type of parameter <pointer>.
// It calls function Struct if <pointer> is type of *struct/**struct.
// It calls function Structs if <pointer> is type of *[]struct/*[]*struct.
//
// The optional parameter <where> is the same as the parameter of Model.WherePri function,
// see Model.WherePri and Model.scan.
//
// Note that it returns sql.ErrNoRows if there's no record retrieved with the given conditions
// from table.
//
