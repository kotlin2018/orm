...
import (
	...
	"github.com/kotlin2018/pkg/gcmd"
	"time"
...
type DB interface {
	...
	// CURD的内部API，可以为自定义CURD实现重写。(公共的CURD APIs就是在它的基础上包装了一层)
	// ===========================================================================
	...
	// ===========================================================================
	// Transaction. Begin()、Transaction()仅由Core结构体实现
	// ===========================================================================
...
// The parameter <master> specifies whether retrieves master node connection if master-slave nodes are configured.
// 检索并返回基础数据库连接对象。如果配置了主从节点，指定参数<master>将检索主节点连接。
